...
### Step 1: Create dedicated VPC and whole components
    ...
    ### VPC
    - Create VPC. 
        create a vpc named `yourname-capstone-vpc` CIDR blok is `10.90.0.0/16` 
        no ipv6 CIDR block
        tenancy: default
    - select `yourname-capstone-vpc` VPC, click `Actions` and `enable DNS hostnames` for the `yourname-capstone-vpc`.
    - do this `this` then
    

    ## Subnets
    - Create Subnets
        - Create a public subnet named `yourname-capstone-public-subnet-1a` under the vpc yourname-capstone-vpc in AZ us-east-1a with 10.90.10.0/24 
        - Create a private subnet named `yourname-capstone-private-subnet-1a` under the vpc yourname-capstone-vpc in AZ us-east-1a with 10.90.11.0/24
        - Create a public subnet named `yourname-capstone-public-subnet-1b` under the vpc yourname-capstone-vpc in AZ us-east-1b with 10.90.20.0/24
        - Create a private subnet named `yourname-capstone-private-subnet-1b` under the vpc yourname-capstone-vpc in AZ us-east-1b with 10.90.21.0/24
    ...
    - Click Internet gateway section on left hand side. Create an internet gateway named `yourname-capstone-igw` and create.

    - ATTACH the internet gateway `yourname-capstone-igw` to the newly created VPC `yourname-capstone-vpc`. Go to VPC and select newly created VPC and click action ---> Attach to VPC ---> Select `yourname-capstone-vpc` VPC 

    ## Route Table
    - Go to route tables on left hand side. We have already one route table as main route table. Change it's name as `yourname-capstone-public-rt` 
    - Create a route table and give a name as `yourname-capstone-private-rt`.
    - Add a rule to `yourname-capstone-public-rt` in which destination 0.0.0.0/0 (any network, any host) to target the internet gateway `yourname-capstone-igw` in order to allow access to the internet.
    ...
    - Name tag      : yourname-capstone-s3-gw-endpoint
    - service name  : `com.amazonaws.us-east-1.s3`
    - type          : Gateway
    - VPC           : `yourname-capstone-vpc`
...
Name            : yourname-capstone-ec2-sg
Description     : EC2 Security Groups only allows traffic coming from yourname-capstone-alb-sg Security Groups for HTTP and HTTPS ports. In addition, ssh port is allowed from anywhere
VPC             : yourname-capstone_VPC
Inbound Rules
HTTP(80)    ----> yourname-capstone-alb-sg
HTTPS (443) ----> yourname-capstone-alb-sg
ssh         ----> anywhere

3. RDS Security Groups
Name            : yourname-capstone-rds-sg
Description     : EC2 Security Groups only allows traffic coming from yourname-capstone-ec2-sg Security Groups for MYSQL/Aurora port. 

VPC             : yourname-capstone-vpc
Inbound Rules
MYSQL/Aurora(3306)  ----> yourname-capstone-ec2-sg

4. NAT Instance Security Group
Name            : yourname-capstone-nat-sg
...
```text
Name               : yourname-capstone-rds-subnet-group
Description        : yourname-capstone-rds-subnet-group
VPC                : yourname-capstone-vpc
Add Subnets
Availability Zones : Select 2 AZ in yourname-capstone-vpc
...
Templates       : Free Tier
Settings        : 
    - DB instance identifier : yourname-capstone-rds
...
Connectivity:
    ...
    VPC                      : yourname-capstone-vpc
    Subnet Group             : yourname-capstone-rds-subnet-group
    Public Access            : No 
    VPC Security Groups      : Choose existing ---> yourname-capstone-rds-sg
...
```text
- Name: yourname-capstone-nat-instance
...
select NAT Instance `amzn-ami-vpc-nat-hvm-2018.03.0.20210915.x86_64-ebs` 
    ...
    - Network : yourname-capstone-vpc
    - Subnet  : yourname-capstone-public-subnet-1a
    - Security Group: yourname-capstone-nat-sg
...
- Launch a new Linux instance in the yourname-capstone-private-1a subnet
...
- Role name: yourname-capstone-ec2-s3-full-access
- Click `Create role`
...
Instance properties:
- Name: yourname-capstone-test-instance
- Ubuntu 22.04
- Key Pair: your key
- Subnet: yourname-capstone-public-1a
- Security Group
    - yourname-capstone-ec2-sg
    - yourname-capstone-alb-sg
    - yourname-capstone-nat-sg
- Advanced details:
    - IAM Instance Profile: 
        - yourname-capstone-ec2-s3-full-access
```bash
sudo su
apt-get update -y
...
```text
Launch template name                : yourname-capstone-launch-template
Template version description        : Blog Web Page version 1
Amazon machine image (AMI)          : Ubuntu 22.04
Instance Type                       : t2.micro
Key Pair                            : your key pair
Subnet                              : Don't include
Security Groups                     : yourname-capstone-ec2-sg
Storage (Volumes)                   : keep it as is
Resource tags                       : Key: Name   Value: yourname-capstone-web-server
Advance Details:
    - IAM instance profile          : yourname-capstone-ec2-s3-full-access
    - User Data
```
```bash
#!/bin/bash
...
Mappings                :
    - Availability zones:
        1. yourname-capstone-public-subnet-1a
        2. yourname-capstone-public-subnet-1b
Security Groups         :
Security Groups         : yourname-capstone-alb-sg

Listeners and Routing   :
Listener HTTPS:443
Protocol HTTP ---> Port 443 ---> Default Action Create Target Group (New Window pops up)
    - Target Type         : Instances
    - Name                : yourname-capstone-tg
    - Protocol            : HTTP
    - Port                : 80
...
```text 
Auto Scaling group name         : yourname-capstone-asg
Launch Template                 : yourname-capstone-launch-template
```
...
- Origin Settings
```text
Origin Domain Name          : yourname-capstone-ALB-1947210493.us-east-2.elb.amazonaws.com
Protocol                    : Match Viewer
HTTP Port                   : 80
HTTPS                       : 443
Minimum Origin SSL Protocol : Keep it as is
Path                 : Leave empty (this means, define for root '/')
Name                        : yourname-capstone-alb-origin
...
```text
Name                : yourname-capstone-health-check
...
Health check            : yourname-capstone-health-check
...
No tags
Role Name           : yourname-capstone-lambda-role
...
- Basic Information
```text
Function Name           : yourname-capstone-lambda-function
Runtime                 : Python 3.10
Permissions
Use an existing role    : yourname-capstone-lambda-role
```
...
- Go to the `yourname-capstone-lambda-function` lambda Function
- Click `Add trigger`
- Select `S3` as Source
...
def lambda_handler(event, context):
    ...
    if event:
        ...
        dynamoTable = dynamo_db.Table('yourname-capstone-dynamo')
